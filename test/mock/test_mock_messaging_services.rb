...
class Mock::TestTwilio < Minitest::Test
  def test_mock_client_messaging_services
    ...
    assert "PN", response.first.sid[0,2]
    assert "AC", response.first.account_sid[0,2]
    assert "service_sid", response.first.service_sid[0,2]
  end
end
